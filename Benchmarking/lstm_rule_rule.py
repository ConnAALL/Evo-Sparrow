...
def lstm_rule_rule_benchmark_core(MODEL_PATH: str,
                                  GAME_COUNT: int,
    ...
    This version is for the LSTM vs. Rule-Based vs. Rule-Based benchmark.
    """
    INPUT_SIZE = 37
    OUTPUT_SIZE = 6
    ...
    def handle_batch_player_move(model, state, key, hidden):
        ...
        # Identify games where the current player is 0 (our LSTM agent)
...
if __name__ == "__main__":
    ...
    with multiprocessing.Pool(processes=num_cores) as pool:
        results_list = pool.starmap(lstm_rule_rule_benchmark_core, pool_args)

    merged_results = merge_results(results_list)

    output_filename = f"lstm_rule_rule_mp_result_{START_DT}.txt"
    with open(output_filename, mode="a") as f:
        f.write("P1 LSTM  P2 Rule-Based  P3 Rule-Based (Multi-Processing)\n")
