...
def ppo_lstm_lstm_benchmark_core(PPO_MODEL_PATH: str,
                                 LSTM_MODEL_PATH: str,
                                 GAME_COUNT: int,
    ...
    This version is for the PPO vs. LSTM vs. LSTM benchmark.
    """
    INPUT_SIZE = 37
    OUTPUT_SIZE = 6
    ...
    def handle_batch_player_move(ppo_model, lstm_model1, lstm_model2, state, key, ppo_hidden, lstm_hidden1, lstm_hidden2):
        ...
        if len(indices_p0) > 0:
            ...
            h0 = ppo_hidden[0][:, indices_p0, :]
            c0 = ppo_hidden[1][:, indices_p0, :]
            ...
            with torch.no_grad():
                ...
                for i in range(len(indices_p0)):
                    ...
                    action, _, _, new_hidden = ppo_model.act(single_input, single_hidden)
                    actions_list.append(action.item())
                    # Update hidden state
                    idx = indices_p0[i]
                    ppo_hidden[0][:, idx, :] = new_hidden[0][:, 0, :]
                    ppo_hidden[1][:, idx, :] = new_hidden[1][:, 0, :]
        ...
        if len(indices_p1) > 0:
            ...
            h0_1 = lstm_hidden1[0][:, indices_p1, :]
            c0_1 = lstm_hidden1[1][:, indices_p1, :]
            device1 = next(lstm_model1.parameters()).device
            input_batch1 = input_batch1.to(device1)
            h0_1 = h0_1.to(device1)
            c0_1 = c0_1.to(device1)
            with torch.no_grad():
                output1, (h_n1, c_n1) = lstm_model1(input_batch1, (h0_1, c0_1))
            ...
            for i, idx in enumerate(indices_p1):
                ...
                lstm_hidden1[0][:, idx, :] = h_n1[:, i, :]
                lstm_hidden1[1][:, idx, :] = c_n1[:, i, :]
        ...
        if len(indices_p2) > 0:
            ...
            h0_2 = lstm_hidden2[0][:, indices_p2, :]
            c0_2 = lstm_hidden2[1][:, indices_p2, :]
            device2 = next(lstm_model2.parameters()).device
            input_batch2 = input_batch2.to(device2)
            h0_2 = h0_2.to(device2)
            c0_2 = c0_2.to(device2)
            with torch.no_grad():
                output2, (h_n2, c_n2) = lstm_model2(input_batch2, (h0_2, c0_2))
            ...
            for i, idx in enumerate(indices_p2):
                ...
                picked_actions[idx] = action
                lstm_hidden2[0][:, idx, :] = h_n2[:, i, :]
                lstm_hidden2[1][:, idx, :] = c_n2[:, i, :]

        return picked_actions, key, ppo_hidden, lstm_hidden1, lstm_hidden2

    def play_games(ppo_model, lstm_model1, lstm_model2):
        init, step = init_env(ENV_ID)
        KEY = reset_random_seed()
        KEY, subkey = jax.random.split(KEY)
        keys = jax.random.split(subkey, GAME_COUNT)
        state = init(keys)
        ppo_hidden = ppo_model.init_hidden(GAME_COUNT)
        lstm_hidden1 = lstm_model1.init_hidden(GAME_COUNT)
        lstm_hidden2 = lstm_model2.init_hidden(GAME_COUNT)

        processed_games = set()
        all_rewards = []
        # Set up a fixed-width progress bar for this core
        progress_bar = tqdm(
            total=GAME_COUNT,
        ...
        while not (state.terminated | state.truncated).all():
            picked_actions, KEY, ppo_hidden, lstm_hidden1, lstm_hidden2 = handle_batch_player_move(
                ppo_model, lstm_model1, lstm_model2, state, KEY, ppo_hidden, lstm_hidden1, lstm_hidden2)
    ...
    ppo_model.load_state_dict(torch.load(PPO_MODEL_PATH))
    ppo_model.eval()

    # Initialize the LSTM models
    lstm_model1 = SparrowMahjongLSTM(
        input_size=INPUT_SIZE,
        hidden_size=HIDDEN_LAYER_SIZE,
    ...
    lstm_model2 = SparrowMahjongLSTM(
        input_size=INPUT_SIZE,
        hidden_size=HIDDEN_LAYER_SIZE,
        output_size=OUTPUT_SIZE,
        num_lstm_layers=LSTM_LAYER_COUNT,
    ...
    lstm_model1.load_state_dict(torch.load(LSTM_MODEL_PATH))
    lstm_model2.load_state_dict(torch.load(LSTM_MODEL_PATH))
    lstm_model1.eval()
    lstm_model2.eval()

    return play_games(ppo_model, lstm_model1, lstm_model2)
...
if __name__ == "__main__":
    ...
    LSTM_MODEL_PATH = "agents/241104_234416_40.pth"
    ...
    for round_num in range(SPLIT):
        ...
        with multiprocessing.Pool(processes=num_cores) as pool:
            round_results = pool.starmap(ppo_lstm_lstm_benchmark_core, pool_args)
    ...
    output_filename = f"ppo_lstm_lstm_mp_result_1750_1M.txt"
    with open(output_filename, mode="a") as f:
        f.write("P1 PPO  P2 LSTM  P3 LSTM (Multi-Processing)\n")
    ...
    print(f"LSTM Agent (Player 2) win rate: {total_player_2_wins / len(merged_results):.4f}")
    print(f"LSTM Agent (Player 3) win rate: {total_player_3_wins / len(merged_results):.4f}") 