...
def lstm_ppo_ppo_benchmark_core(LSTM_MODEL_PATH: str,
                                PPO_MODEL_PATH: str,
                                GAME_COUNT: int,
    ...
    This version is for the LSTM vs. PPO vs. PPO benchmark.
    """
    INPUT_SIZE = 37
    OUTPUT_SIZE = 6
    ...
    def handle_batch_player_move(lstm_model, ppo_model1, ppo_model2, state, key, lstm_hidden, ppo_hidden1, ppo_hidden2):
        ...
        # --- Process LSTM agent moves (player 0) ---
        ...
        if len(indices_p0) > 0:
            ...
            h0 = lstm_hidden[0][:, indices_p0, :]
            c0 = lstm_hidden[1][:, indices_p0, :]
            device = next(lstm_model.parameters()).device
            input_batch = input_batch.to(device)
            h0 = h0.to(device)
            c0 = c0.to(device)
            with torch.no_grad():
                output, (h_n, c_n) = lstm_model(input_batch, (h0, c0))
            ...
            for i, idx in enumerate(indices_p0):
                ...
                # Update hidden state
                lstm_hidden[0][:, idx, :] = h_n[:, i, :]
                lstm_hidden[1][:, idx, :] = c_n[:, i, :]
        ...
        if len(indices_p1) > 0:
            ...
            h0_1 = ppo_hidden1[0][:, indices_p1, :]
            c0_1 = ppo_hidden1[1][:, indices_p1, :]
            device1 = next(ppo_model1.parameters()).device
        ...
        if len(indices_p2) > 0:
            ...
            for idx in indices_p2:
                ...
                input_array = state_to_input(state, idx, 2)  # Use 3-parameter version
                input_tensor = torch.tensor(input_array, dtype=torch.float32).unsqueeze(0).unsqueeze(0)  # PPO needs (batch, seq, features)
                input_tensors2.append(input_tensor)
            input_batch2 = torch.cat(input_tensors2, dim=0)  # Concatenate along batch dimension
            h0_2 = ppo_hidden2[0][:, indices_p2, :]
            c0_2 = ppo_hidden2[1][:, indices_p2, :]
            device2 = next(ppo_model2.parameters()).device
            ...
            with torch.no_grad():
                ...
                for i in range(len(indices_p2)):
                    single_input = input_batch2[i:i+1]  # Keep batch dimension
        ...
        return picked_actions, key, lstm_hidden, ppo_hidden1, ppo_hidden2

    def play_games(lstm_model, ppo_model1, ppo_model2):
        init, step = init_env(ENV_ID)
        KEY = reset_random_seed()
        KEY, subkey = jax.random.split(KEY)
        keys = jax.random.split(subkey, GAME_COUNT)
        state = init(keys)
        lstm_hidden = lstm_model.init_hidden(GAME_COUNT)
        ...
        while not (state.terminated | state.truncated).all():
            picked_actions, KEY, lstm_hidden, ppo_hidden1, ppo_hidden2 = handle_batch_player_move(
                lstm_model, ppo_model1, ppo_model2, state, KEY, lstm_hidden, ppo_hidden1, ppo_hidden2)
    ...
    return play_games(lstm_model, ppo_model1, ppo_model2)
...
if __name__ == "__main__":
    ...
    for round_num in range(SPLIT):
        ...
        with multiprocessing.Pool(processes=num_cores) as pool:
            round_results = pool.starmap(lstm_ppo_ppo_benchmark_core, pool_args)
    ...
    output_filename = f"lstm_ppo_ppo_mp_result_1750_1M.txt"
    with open(output_filename, mode="a") as f:
        f.write("P1 LSTM  P2 PPO  P3 PPO (Multi-Processing)\n")
        f.write("Games Played: " + str(len(merged_results)) + "\n")

        ### Player 1 Results (LSTM) ###
    ...
    print(f"LSTM Agent (Player 1) win rate: {player_1_average_wins:.4f}")
