...
def ppo_lstm_random_benchmark_core(PPO_MODEL_PATH: str,
                                   LSTM_MODEL_PATH: str,
                                   GAME_COUNT: int,
    ...
    This version is for the PPO vs. LSTM vs. Random benchmark.
    """
    INPUT_SIZE = 37
    OUTPUT_SIZE = 6
    ...
    def handle_batch_player_move(ppo_model, lstm_model, state, key, ppo_hidden, lstm_hidden):
        ...
        if len(indices_p1) > 0:
            ...
            h0_1 = lstm_hidden[0][:, indices_p1, :]
            c0_1 = lstm_hidden[1][:, indices_p1, :]
            device1 = next(lstm_model.parameters()).device
            input_batch1 = input_batch1.to(device1)
            h0_1 = h0_1.to(device1)
            c0_1 = c0_1.to(device1)
            with torch.no_grad():
                output1, (h_n1, c_n1) = lstm_model(input_batch1, (h0_1, c0_1))
            ...
            for i, idx in enumerate(indices_p1):
                ...
                lstm_hidden[0][:, idx, :] = h_n1[:, i, :]
                lstm_hidden[1][:, idx, :] = c_n1[:, i, :]

        # --- Process Random agent for Player 2 ---
        ...
        return picked_actions, key, ppo_hidden, lstm_hidden

    def play_games(ppo_model, lstm_model):
        ...
        while not (state.terminated | state.truncated).all():
            picked_actions, KEY, ppo_hidden, lstm_hidden = handle_batch_player_move(
                ppo_model, lstm_model, state, KEY, ppo_hidden, lstm_hidden)
    ...
    return play_games(ppo_model, lstm_model)
...
if __name__ == "__main__":
    ...
    for round_num in range(SPLIT):
        ...
        with multiprocessing.Pool(processes=num_cores) as pool:
            round_results = pool.starmap(ppo_lstm_random_benchmark_core, pool_args)
    ...
    output_filename = f"ppo_lstm_random_mp_result_1750_1M.txt"
    with open(output_filename, mode="a") as f:
        f.write("P1 PPO  P2 LSTM  P3 Random (Multi-Processing)\n")
    ...
    print(f"Random Agent (Player 3) win rate: {total_player_3_wins / len(merged_results):.4f}") 