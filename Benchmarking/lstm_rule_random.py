...
def lstm_rule_random_benchmark_core(MODEL_PATH: str,
                                    GAME_COUNT: int,
                                    ...
                                    core_id: int):
    """
    Run the benchmark for a subset of games on one core.
    """
    INPUT_SIZE = 37
    OUTPUT_SIZE = 6
    ...
    def handle_batch_player_move(model, state, key, hidden):
        """
        Handle moves for a batch of games for LSTM vs. Rule vs. Random:
         - Player 0: LSTM agent
        ...
        player_lstm = (state.current_player == 0) & active_games
        player_rule = (state.current_player == 1) & active_games
        player_random = (state.current_player == 2) & active_games

        # Process LSTM agent moves (Player 0)
        indices_lstm = np.where(player_lstm)[0]
        if len(indices_lstm) > 0:
            input_tensors = []
            valid_moves_list = []
            for idx in indices_lstm:
            ...
            h0 = hidden[0][:, indices_lstm, :]
            c0 = hidden[1][:, indices_lstm, :]
            ...
            for i, idx in enumerate(indices_lstm):
                ...
                # Update hidden state for LSTM agent.
                hidden[0][:, idx, :] = h_n[:, i, :]
                hidden[1][:, idx, :] = c_n[:, i, :]

        # Process Rule-based agent moves (Player 1)
        indices_rule = np.where(player_rule)[0]
        for idx in indices_rule:
            actions = get_valid_actions(state, idx, 1)
        ...
        # Process Random agent moves (Player 2)
        indices_random = np.where(player_random)[0]
    ...
    def play_games(model):
        ...
        processed_games = set()
        all_rewards = []
        # Set up tqdm progress bar for this core.
        progress_bar = tqdm(
            total=GAME_COUNT,
            desc=f"Core {core_id}".ljust(8),  # Ensures all labels are the same width
            position=core_id,
            leave=True,
            ncols=100,  # Adjust the width of the progress bar (change this value as needed),
        ...
        while not (state.terminated | state.truncated).all():
            ...
            # Update the progress bar by the number of newly finished games.
...
if __name__ == '__main__':
    ...
    with multiprocessing.Pool(processes=num_cores) as pool:
        results = pool.starmap(lstm_rule_random_benchmark_core, pool_args)

    merged_results = merge_results(results)

    output_filename = f"lstm_rule_random_mp_result_{MODEL_START_DT}.txt"
    with open(output_filename, mode="a") as f:
        f.write("P1 LSTM  P2 Rule-Based  P3 Random (Multi-Processing)\n")
