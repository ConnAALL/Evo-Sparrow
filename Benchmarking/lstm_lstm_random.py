...
def lstm_lstm_random_benchmark_core(MODEL_PATH: str,
                                      GAME_COUNT: int,
                                      ...
                                      core_id: int):
    """
    Run the benchmark for a subset of games on one core using two LSTM agents (for players 0 and 1)
    and one random agent (player 2).
    """
    INPUT_SIZE = 37
    OUTPUT_SIZE = 6
    ...
    def get_valid_actions(state, idx, player_idx):
        """Get valid actions for a player (with red Dora represented as negative indices)."""
        ...
        for action in valid_indices:
            ...
            # Append red Dora moves as negative
    ...
    def handle_batch_player_move(model0, model1, state, key, hidden0, hidden1):
        """
        For a batch of games, handle moves for:
        ...
        if len(indices_p0) > 0:
            ...
            for idx in indices_p0:
                ...
                # (Ensure that state_to_input returns a vector of length 37)
...
if __name__ == '__main__':
    ...
    with multiprocessing.Pool(processes=num_cores) as pool:
        results = pool.starmap(lstm_lstm_random_benchmark_core, pool_args)

    merged_results = merge_results(results)

    output_filename = f"lstm_lstm_random_mp_result_{MODEL_START_DT}.txt"
    with open(output_filename, mode="a") as f:
        f.write("P1 LSTM  P2 LSTM  P3 Random (Multi-Processing)\n")
