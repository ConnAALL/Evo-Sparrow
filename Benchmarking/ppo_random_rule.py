...
def ppo_random_rule_benchmark_core(PPO_MODEL_PATH: str,
                                   GAME_COUNT: int,
    ...
    This version is for the PPO vs. Random vs. Rule-based benchmark.
    """
    INPUT_SIZE = 37
    OUTPUT_SIZE = 6
    ...
    def handle_batch_player_move(ppo_model, state, key, ppo_hidden):
        ...
        # --- Process Random agent for Player 1 ---
        ...
        return picked_actions, key, ppo_hidden

    def play_games(ppo_model):
        ...
        while not (state.terminated | state.truncated).all():
            picked_actions, KEY, ppo_hidden = handle_batch_player_move(
                ppo_model, state, KEY, ppo_hidden)
...
if __name__ == "__main__":
    ...
    for round_num in range(SPLIT):
        ...
        with multiprocessing.Pool(processes=num_cores) as pool:
            round_results = pool.starmap(ppo_random_rule_benchmark_core, pool_args)
    ...
    output_filename = f"ppo_random_rule_mp_result_1750_1M.txt"
    with open(output_filename, mode="a") as f:
        f.write("P1 PPO  P2 Random  P3 Rule-Based (Multi-Processing)\n")
    ...
    print(f"Random Agent (Player 2) win rate: {total_player_2_wins / len(merged_results):.4f}")
