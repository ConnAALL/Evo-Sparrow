...
def ppo_random_random_benchmark_core(MODEL_PATH: str,
                                     GAME_COUNT: int,
                                     ...
                                     core_id: int):
    """
    Run the PPO-Random-Random benchmark for a subset of games on one core.
    """
    INPUT_SIZE = 37
    OUTPUT_SIZE = 6
    ...
    def handle_batch_player_move(model, state, key, hidden):
        ...
        # Identify games where the current player is 0 (PPO agent)
        player1_turns = (state.current_player == 0) & active_games

        # Process PPO agent moves for player 0
    ...
    ppo_model.eval()
    all_rewards = play_games(ppo_model)
    return all_rewards
...
if __name__ == '__main__':
    ...
    with multiprocessing.Pool(processes=num_cores) as pool:
        results = pool.starmap(ppo_random_random_benchmark_core, pool_args)

    merged_results = merge_results(results)

    output_filename = f"ppo_random_random_mp_result_1750_1M.txt"
    with open(output_filename, mode="a") as f:
        f.write("P1 PPO  P2 Random  P3 Random (Multi-Processing)\n")
        f.write("Games Played: " + str(len(merged_results)) + "\n")

        # --- Player 1 (PPO) Results ---
