...
def lstm_lstm_rule_benchmark_core(MODEL_PATH: str,
                                    GAME_COUNT: int,
    ...
    Run the benchmark on one core with:
      - Player 0: LSTM agent (using model0)
      - Player 1: LSTM agent (using model1)
      - Player 2: Rule-based agent.
    A fixed-width tqdm progress bar (positioned by core_id) shows progress.
    """
    INPUT_SIZE = 37
    OUTPUT_SIZE = 6
    ...
    def handle_batch_player_move(model0, model1, state, key, hidden0, hidden1):
        """
        ...
        if len(indices_p0) > 0:
            ...
            for idx in indices_p0:
                ...
                # Note: Using the three-argument version to get player 0's state.
        ...
        if len(indices_p1) > 0:
            ...
            for idx in indices_p1:
                ...
                # Use the three-argument version for player 1.
        ...
        player_rule = (state.current_player == 2) & active_games
        indices_rule = np.where(player_rule)[0]
        for idx in indices_rule:
            # Use get_valid_actions with player index 2.
            actions = get_valid_actions(state, idx, 2)
            hand = actions_to_hand(actions)
            # For the rule-based agent, determine the next move using the rule functions.
        ...
        return picked_actions, key, hidden0, hidden1

    def play_games(model0, model1):
        ...
        while not (state.terminated | state.truncated).all():
            picked_actions, KEY, hidden0, hidden1 = handle_batch_player_move(model0, model1, state, KEY, hidden0, hidden1)
    ...
    # Initialize and load the two LSTM models.
    ...
    # Load saved weights (using the same file for both networks; change if needed).
    lstm_model_p0.load_state_dict(torch.load(MODEL_PATH))
    lstm_model_p1.load_state_dict(torch.load(MODEL_PATH))
    lstm_model_p0.eval()
    lstm_model_p1.eval()

    all_rewards = play_games(lstm_model_p0, lstm_model_p1)
    return all_rewards
...
if __name__ == '__main__':
    ...
    with multiprocessing.Pool(processes=num_cores) as pool:
        results = pool.starmap(lstm_lstm_rule_benchmark_core, pool_args)

    merged_results = merge_results(results)

    output_filename = f"lstm_lstm_rule_mp_result_{MODEL_START_DT}.txt"
    with open(output_filename, mode="a") as f:
        f.write("P1 LSTM  P2 LSTM  P3 Rule-Based (Multi-Processing)\n")
